...
/*   Updated: 2024/12/23 15:21:54 by emonacho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	remainder[fd] = read_and_store_fd(fd, remainder[fd]);
	if (remainder[fd] == 0)
		return (NULL);
	line = is_current_line(remainder[fd]);
	remainder[fd] = save_leftovers(remainder[fd]);
	return (line);
}
